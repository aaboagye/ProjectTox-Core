...
static struct Client_data  close_clientlist[LCLIENT_LIST];
static struct Friend      *friends_list;
static uint16_t     num_friends;
static struct Pinged       send_nodes[LSEND_NODES_ARRAY];
 ...
 */
static int client_in_list(struct Client_data *list, uint32_t length, uint8_t *client_id, IP_Port ip_port)
{
    uint32_t i;
 ...
 */
static int replace_bad(    struct Client_data    *list,
                           uint32_t        length,
                           uint8_t        *client_id,
                           IP_Port         ip_port )
{
...
static void sort_list(struct Client_data *list, uint32_t length, uint8_t *comp_client_id)
{
    if (length == 0)
        return;
    ...
    while (1) {
        ...
        for (i = 0; i < (length - 1); ++i) {
            if (id_closest(comp_client_id, list[i].client_id, list[i + 1].client_id) == 1) {
                struct Client_data temp = list[i + 1];
                list[i + 1] = list[i];
                list[i] = temp;
                ++count;
            }
...
static int replace_good(   struct Client_data    *list,
                           uint32_t        length,
                           uint8_t        *client_id,
                           IP_Port         ip_port,
...
int DHT_addfriend(uint8_t *client_id)
{
    ...
    struct Friend *temp;
    temp = realloc(friends_list, sizeof(struct Friend) * (num_friends + 1));

    if (temp == NULL)
        return 1;

    friends_list = temp;
    memset(&friends_list[num_friends], 0, sizeof(struct Friend));
...
int DHT_delfriend(uint8_t *client_id)
{
    ...
    for (i = 0; i < num_friends; ++i) {
        ...
        if (id_equal(friends_list[i].client_id, client_id)) {
            ...
            temp = realloc(friends_list, sizeof(struct Friend) * (num_friends));
...
static int handle_NATping(IP_Port source, uint8_t *source_pubkey, uint8_t *packet, uint32_t length)
{
    ...
    struct Friend *friend = &friends_list[friendnumber];
...
{
    return sizeof(close_clientlist) + sizeof(struct Friend) * num_friends;
}
...
void DHT_save(uint8_t *data)
{
    ...
    memcpy(data + sizeof(close_clientlist), friends_list, sizeof(struct Friend) * num_friends);
}
...
int DHT_load(uint8_t *data, uint32_t size)
{
    ...
    if ((size - sizeof(close_clientlist)) % sizeof(struct Friend) != 0)
        return -1;
    ...
    temp = (size - sizeof(close_clientlist)) / sizeof(struct Friend);

    if (temp != 0) {
        struct Friend *tempfriends_list = (struct Friend *)(data + sizeof(close_clientlist));
    ...
    struct Client_data *tempclose_clientlist = (struct Client_data *)data;
