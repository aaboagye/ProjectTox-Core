...
int realloc_friendlist(struct Messenger *m, uint32_t num)
{
    struct Friend *newfriendlist = realloc(m->friendlist, num * sizeof(struct Friend));

    if (newfriendlist == NULL)
        return -1;

    memset(&newfriendlist[num - 1], 0, sizeof(struct Friend));
...
int getfriend_id(struct Messenger *m, uint8_t *client_id)
{
    uint32_t i;
...
/* copies the public key associated to that friend id into client_id buffer.
   ...
   return 0 if success
   return -1 if failure. */
int getclient_id(struct Messenger *m, int friend_id, uint8_t *client_id)
{
 ...
 *
 */
void getaddress(struct Messenger *m, uint8_t *address)
{
 ...
 */
int m_addfriend(struct Messenger *m, uint8_t *address, uint8_t *data, uint16_t length)
{
...
int m_addfriend_norequest(struct Messenger *m, uint8_t *client_id)
{
...
/* remove a friend
   return 0 if success
   return -1 if failure */
int m_delfriend(struct Messenger *m, int friendnumber)
{
    ...
    memset(&(m->friendlist[friendnumber]), 0, sizeof(struct Friend));
    uint32_t i;
...
int m_friendstatus(struct Messenger *m, int friendnumber)
{
...
uint32_t m_sendmessage(struct Messenger *m, int friendnumber, uint8_t *message, uint32_t length)
{
    if (friendnumber < 0 || friendnumber >= m->numfriends)
        return 0;
...
uint32_t m_sendmessage_withid(struct Messenger *m, int friendnumber, uint32_t theid, uint8_t *message, uint32_t length)
{
...
/* send an action to an online friend
   ...
   return 0 if it was not */
int m_sendaction(struct Messenger *m, int friendnumber, uint8_t *action, uint32_t length)
{
...
static int m_sendname(struct Messenger *m, int friendnumber, uint8_t *name, uint16_t length)
{
...
/* set the name of a friend
   return 0 if success
   return -1 if failure */
static int setfriendname(struct Messenger *m, int friendnumber, uint8_t *name)
{
    if (friendnumber >= m->numfriends || friendnumber < 0)
        return -1;
...
/* Set our nickname
   ...
   return 0 if success
   return -1 if failure */
int setname(struct Messenger *m, uint8_t *name, uint16_t length)
{
    if (length > MAX_NAME_LENGTH || length == 0)
        return -1;
...
uint16_t getself_name(struct Messenger *m, uint8_t *name, uint16_t nlen)
{
    uint16_t len;
...
/* get name of friendnumber
   ...
   return 0 if success
   return -1 if failure */
int getname(struct Messenger *m, int friendnumber, uint8_t *name)
{
    if (friendnumber >= m->numfriends || friendnumber < 0)
        return -1;
...
int m_set_statusmessage(struct Messenger *m, uint8_t *status, uint16_t length)
{
...
int m_set_userstatus(struct Messenger *m, enum USERSTATUS status)
{
...
int m_get_statusmessage_size(struct Messenger *m, int friendnumber)
{
    if (friendnumber >= m->numfriends || friendnumber < 0)
        return -1;
...
int m_copy_statusmessage(struct Messenger *m, int friendnumber, uint8_t *buf, uint32_t maxlen)
{
    if (friendnumber >= m->numfriends || friendnumber < 0)
        return -1;
...
int m_copy_self_statusmessage(struct Messenger *m, uint8_t *buf, uint32_t maxlen)
{
    memset(buf, 0, maxlen);
...
enum USERSTATUS m_get_userstatus(struct Messenger *m, int friendnumber)
{
    if (friendnumber >= m->numfriends || friendnumber < 0)
        return USERSTATUS_INVALID;

    enum USERSTATUS status = m->friendlist[friendnumber].userstatus;
...
enum USERSTATUS m_get_self_userstatus(struct Messenger *m)
{
    return m->userstatus;
}

static int send_statusmessage(struct Messenger *m, int friendnumber, uint8_t *status, uint16_t length)
{
    return write_cryptpacket_id(m, friendnumber, PACKET_ID_STATUSMESSAGE, status, length);
}

static int send_userstatus(struct Messenger *m, int friendnumber, enum USERSTATUS status)
{
    uint8_t stat = status;
    return write_cryptpacket_id(m, friendnumber, PACKET_ID_USERSTATUS, &stat, sizeof(stat));
}

static int set_friend_statusmessage(struct Messenger *m, int friendnumber, uint8_t *status, uint16_t length)
{
    if (friendnumber >= m->numfriends || friendnumber < 0)
        return -1;
...
static void set_friend_userstatus(struct Messenger *m, int friendnumber, enum USERSTATUS status)
{
    m->friendlist[friendnumber].userstatus = status;
}

/* Sets whether we send read receipts for friendnumber. */
void m_set_sends_receipts(struct Messenger *m, int friendnumber, int yesno)
{
    if (yesno != 0 || yesno != 1)
        return;
...
void m_callback_friendrequest(struct Messenger *m, void (*function)(uint8_t *, uint8_t *, uint16_t, void *),
                              void *userdata)
{
...
void m_callback_friendmessage(struct Messenger *m, void (*function)(struct Messenger *m, int, uint8_t *, uint16_t,
                              void *), void *userdata)
{
...
void m_callback_action(struct Messenger *m, void (*function)(struct Messenger *m, int, uint8_t *, uint16_t, void *),
                       void *userdata)
{
...
void m_callback_namechange(struct Messenger *m, void (*function)(struct Messenger *m, int, uint8_t *, uint16_t, void *),
                           void *userdata)
{
...
void m_callback_statusmessage(struct Messenger *m, void (*function)(struct Messenger *m, int, uint8_t *, uint16_t,
                              void *), void *userdata)
{
...
void m_callback_userstatus(struct Messenger *m, void (*function)(struct Messenger *m, int, enum USERSTATUS, void *),
                           void *userdata)
{
...
void m_callback_read_receipt(struct Messenger *m, void (*function)(struct Messenger *m, int, uint32_t, void *),
                             void *userdata)
{
...
void m_callback_connectionstatus(struct Messenger *m, void (*function)(struct Messenger *m, int, uint8_t, void *),
                                 void *userdata)
{
...
static void check_friend_connectionstatus(struct Messenger *m, int friendnumber, uint8_t status)
{
...
void set_friend_status(struct Messenger *m, int friendnumber, uint8_t status)
{
    check_friend_connectionstatus(m, friendnumber, status);
    m->friendlist[friendnumber].status = status;
}

int write_cryptpacket_id(struct Messenger *m, int friendnumber, uint8_t packet_id, uint8_t *data, uint32_t length)
{
    if (friendnumber < 0 || friendnumber >= m->numfriends)
        return 0;
...
int LANdiscovery(struct timer *t, void *arg)
{
...
/* run this at startup */
struct Messenger *initMessenger(void)
{
    struct Messenger *m = calloc(1, sizeof(struct Messenger));
...
void cleanupMessenger(struct Messenger *m)
{
...
void doFriends(struct Messenger *m)
{
    ...
    for (i = 0; i < m->numfriends; ++i) {
        ...
        if (m->friendlist[i].status == FRIEND_REQUESTED
            ...
            struct struct IP_Port friendip = DHT_getfriendip(m->friendlist[i].client_id);
...
void doInbound(struct Messenger *m)
{
...
void doMessenger(struct Messenger *m)
{
    networking_poll();
...
uint32_t Messenger_size(struct Messenger *m)
{
    return crypto_box_PUBLICKEYBYTES + crypto_box_SECRETKEYBYTES + sizeof(uint32_t)
           + sizeof(uint32_t) + DHT_size() + sizeof(uint32_t) + sizeof(struct Friend) * m->numfriends;
}

/* save the messenger in data of size Messenger_size() */
void Messenger_save(struct Messenger *m, uint8_t *data)
{
    save_keys(data);
    ...
    DHT_save(data);
    data += size;
    size = sizeof(struct Friend) * m->numfriends;
    memcpy(data, &size, sizeof(size));
    data += sizeof(size);
    memcpy(data, m->friendlist, sizeof(struct Friend) * m->numfriends);
}

/* load the messenger from data of size length. */
int Messenger_load(struct Messenger *m, uint8_t *data, uint32_t length)
{
    if (length == ~0)
        return -1;
    ...
    if (length != size || length % sizeof(struct Friend) != 0)
        return -1;

    struct Friend *temp = malloc(size);
    memcpy(temp, data, size);

    uint16_t num = size / sizeof(struct Friend);
